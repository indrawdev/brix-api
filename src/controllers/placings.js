...
exports.listHeadPlacings = async (req, res, next) => {
	...
	await Placing.findAndCountAll({
		...
		attributes: ['placing_id', 'placing_code', 'type_cover', 'insured_person', 'start_periode', 'end_periode', 'policy_number', 'brokerage_pct', 'currency'],
		where: {
			is_active: '1',
			placing_code: {
				[Op.like]: `%${search}%`
...
exports.listOwnPlacings = async (req, res, next) => {
	let offset = parseInt(req.query.offset) || 0
	let limit = parseInt(req.query.limit) || 10
	let search = req.query.search || ''
